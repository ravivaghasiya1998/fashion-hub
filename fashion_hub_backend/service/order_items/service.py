...
class OrderItemService:
    ...
    def create_order_item(self, order_item: schemas.OrderedItemCreate):
        order_exists = self.db.scalars(
            order_models.Orders.select().where(order_models.Orders.id == order_item.order_id)
        ).first()
        if not order_exists:
            raise APIBadRequest(detail=f"Order with id: {order_item.order_id} does not exist")
        product_exists = self.db.scalars(
            product_models.Products.select().where(product_models.Products.id == order_item.product_id)
        ).first()
        if not product_exists:
            raise APIBadRequest(detail=f"Product with id: {order_item.product_id} does not exist")
    ...
    def get_order_item(self, order_item_id: int):
        order_item = self.db.scalars(models.Orders.select().where(models.OrderedItem.id == order_item_id)).first()
        if not order_item:
            raise APINotFound(detail=f"Order item with id: {order_item_id} does not exist")
    ...
    def get_order_items(self):
        ...
        return [schemas.OrderedItem.model_validate(order, from_attributes=True) for order in orders]

    def update_order_item(self, order_item_id: int, order_item: schemas.OrderedItemUpdate):
        ...
        return schemas.OrderedItem.model_validate(order_items_exists, from_attributes=True)

    def delete_order_item(self, order_item_id: int):
        order_item = self.db.scalars(models.OrderedItem.select().where(models.OrderedItem.id == order_item_id)).first()
